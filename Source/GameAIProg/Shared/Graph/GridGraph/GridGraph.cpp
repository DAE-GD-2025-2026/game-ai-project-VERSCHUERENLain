...
GridGraph::GridGraph(IGraphNodeFactory* Factory, int Rows, int Cols, float InCellSize, float Cost,
...
{
	return {Index % NrColumns, Index / NrColumns};
}
...
void GridGraph::DebugDrawCells(UWorld const* const World) const
{
	...
	FVector GridOrigin3D{GridOrigin, 0.0f};
	for (int Row = 0; Row < NrRows; ++Row)
	{
		for (int Col = 0; Col < NrColumns; ++Col)
		{
			FVector const NodePos{CellSize * Col + CellExtents.X, CellSize * Row + CellExtents.Y, 1.0f};
			DrawDebugBox(World, GridOrigin3D + NodePos, CellExtents, FColor::Red);
		}
	}
}
